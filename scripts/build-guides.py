...
def render(filename,title,intro,body,day=None):
 ...
 header=D['header'].replace('Travel toolkit','Documents & essentials').replace('Taste & explore','General recommendations')
 nav=''
 if day:
  i=day['id'];nav='<nav class="guide-nav" aria-label="Day navigation">'+link(f'./index.html#day-{i}','← Back to this day’s itinerary')
  if i>1:nav+=link(f'./day-{i-1:02}.html','← Previous day')
  if i<11:nav+=link(f'./day-{i+1:02}.html','Next day →')
  nav+='</nav>'
...
for day in D['days']:
 i=day['id'];body='<nav class="guide-nav" aria-label="On this day">'+link('#recommendations','Food & experiences')+link('#useful','Useful today')+link('#hotels','Hotel & laundry')+'</nav>'
